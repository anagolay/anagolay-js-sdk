 ...
 * @param signer Who will be signing the transaction and paying fees?
 */
export async function saveBulk(d: AnAnagolayStatement[], signer: KeyringPair): Promise<CustomEventEmitter> {
 ...
 */
export function createCorrectSubmittableExtrinsic(d: AnAnagolayStatement): SubmittableExtrinsic<'promise'> {
  switch (d.data.claim.claimType) {
    case AnAnagolayClaimType.COPYRIGHT:
 ...
 */
export async function saveCopyright(d: AnAnagolayStatement, signer: KeyringPair): Promise<EventEmitter> {
  const broadcast = createEventEmitter();
 ...
 * @param signer Account that will be owner of the transaction and ones who pays the fees
 */
export async function saveOwnership(d: AnAnagolayStatement, signer: KeyringPair): Promise<EventEmitter> {
  const broadcast = createEventEmitter();
 ...
 * @param statementId
 */
export function createSubmittableExtrinsicRevoke(statementId: AnGenericId): SubmittableExtrinsic<'promise'> {
  const api = getApi();
 ...
 */
export async function revokeStatementsBulk(d: AnGenericId[], signer: KeyringPair): Promise<EventEmitter> {
  const api = getApi();
  const broadcast = createEventEmitter();
