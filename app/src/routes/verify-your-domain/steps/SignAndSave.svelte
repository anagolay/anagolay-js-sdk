...
async function subToEvents() {
  pallets.verification.listenForEvent('VerificationSuccessful').once('VerificationSuccessful', async (d) => {
...
onDestroy(() => {
  // this will stop the confetti
  $mainStore.statementCreated = false;
});
</script>
    ...
    {:else}
      <div class="flex flex-col gap-4">
        ...
        <button
          ...
          disabled="{$mainStore.statementCreated ||
            $mainStore.savingStatement ||
            !$mainStore.canSaveStatement}"
